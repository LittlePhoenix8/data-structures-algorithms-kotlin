 ...
 * themselves or to another student), and then they'll pass it to student arr[i-1].
 * It's possible that arr[i-1] = i for any given i, in which case student i will pass their yearbook back to themselves.
 * Once a student has received their own yearbook back, they will hold on to it and no longer participate in the passing
 * process. It's guaranteed that, for any possible valid input, each student will eventually receive their own yearbook
 ...
 * */
fun main() {
    val passingYearbooks = PassingYearbooks()
    println(passingYearbooks.findSignatureCounts(arrayOf(2, 1)).joinToString(",")) //[2, 2]
    println(passingYearbooks.findSignatureCounts(arrayOf(1, 2)).joinToString(",")) //[1, 1]
}
