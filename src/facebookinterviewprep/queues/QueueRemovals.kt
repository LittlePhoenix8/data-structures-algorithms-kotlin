package facebookinterviewprep.queues
 ...
 * Of the elements that were popped, find the one with the largest value (if there are multiple such elements, take the
 * one which had been popped the earliest), and remove it
 * For each one of the remaining elements that were popped (in the order they had been popped), decrement its value by
 * 1 if it's positive (otherwise, if its value is 0, then it's left unchanged), and then add it back to the queue
 ...
 * are added back onto the queue, whose contents are then [0, 0].
 ...
 * */
fun main() {
    val queueRemovals = QueueRemovals()
    println(queueRemovals.findPositions(arrayOf(1, 2, 2, 3, 4, 5), 5).joinToString(", ")) //[5, 6, 4, 1, 2]
    println(queueRemovals.findPositions(arrayOf(2, 4, 2, 4, 3, 1, 2, 2, 3, 4, 3, 4, 4), 4).joinToString(", ")) //[2, 5, 10, 13]
}

class QueueRemovals {
    fun findPositions(arr: Array<Int>, x: Int): Array<Int> {
        val result = Array(x) { 0 }
        val queue = LinkedList<Number>()
        //O(n)
        for(i in arr.indices) {
            queue.add(Number(arr[i], i + 1))
        }
        //O(x^(2x))
        for(i in 1..x) {
            val n = if(queue.size > x) x else queue.size
            val pop = Array(n) { Number(0, 0) }
            var big = Number(Integer.MIN_VALUE, Integer.MAX_VALUE)
            for(j in 0 until n) {
                pop[j] = queue.remove()
                if(pop[j].number > big.number) {
                    big = pop[j]
                }
            }
            result[i - 1] = big.index
            for(j in pop.indices) {
                if(pop[j].index != big.index) {
                    queue.add(Number(if(pop[j].number > 0) pop[j].number - 1 else pop[j].number, pop[j].index))
                }
            }
        }
        return result
    }
    class Number(var number: Int, var index: Int)
}